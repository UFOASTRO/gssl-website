...
const BASE_CONFIG: Omit<COBEOptions, 'width' | 'height' | 'devicePixelRatio' | 'mapSamples'> = {
  phi: 0,
  theta: 0.25,
...
function getDeviceTier() {
  ...
  const slowConn = ['slow-2g', '2g', '3g'].includes(connection?.effectiveType ?? '');
  const reducedMotion =
...
export default function Globe() {
  ...
  const dragRef = useRef<{ x: number; y: number; phiAtStart: number; thetaAtStart: number } | null>(
    null
  );
  // Last pointer sample, used to measure release velocity for momentum.
  const lastMoveRef = useRef<{ x: number; y: number; t: number; vx: number; vy: number } | null>(
    null
  );
  ...
  const onPointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    lastMoveRef.current = { x: e.clientX, y: e.clientY, t: performance.now(), vx: 0, vy: 0 };
  ...
  const onPointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    phiRef.current = dragRef.current.phiAtStart + totalDeltaX / ROTATION_SENSITIVITY;
    thetaRef.current = clamp(
    ...
    <div ref={containerRef} className="globe-container">
      ...
      {MARKERS.map((m) => (
        ...
        >
          ...
          {hoveredId === m.id && <span className="globe-label__detail">{m.description}</span>}
        </button>
      ))}
    </div>
  );
