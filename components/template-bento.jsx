...
function computeWeights(count, activeIndex, expand) {
  ...
  return Array.from({ length: count }, (_, i) => (i === activeIndex ? expand : otherWeight));
}
...
function HoverBentoGrid({
  ...
  const activeRow = activeIndex === null ? null : Math.floor(activeIndex / effColumns);
  ...
  const rootVars = {
    ...
    "--hbg-border-c": border ? border.color ?? "rgba(255,255,255,0.1)" : "transparent",
  };

  return (
    <div className={`hbg-root ${className ?? ""}`} style={{ ...rootVars, ...style }}>
      <style>{css(uid)}</style>
      <div
...
function Tile({ tile, isActive, isStatic, onEnter, onLeave, onToggle }) {
  ...
  return (
    <div
      ...
      aria-label={[tile.title, tile.tag, tile.description].filter(Boolean).join(". ")}
          ...
          <div className="hbg-media">
            {base && (
              <img
                ...
                src={base.src}
                alt={base.alt ?? ""}
                style={{ "--fit": base.fit ?? "cover", transform: isActive ? tile.transform : undefined }}
              />
            )}
            {hover && (
              <img
              ...
              <div className="hbg-reveal-inner">
                ...
                {tile.link && (
                  <a
                    ...
                    rel={tile.link.target === "_blank" ? "noopener noreferrer" : undefined}
...
const img = (seed, w = 1000, h = 800) => `https://picsum.photos/seed/${seed}/${w}/${h}`;
...
const tiles = [
  ...
  },
  {
    ...
    description: "Bark, salt flats, worn stone steps — surfaces that reward getting close and staying still.",
    link: { href: "#", label: "View series" },
  },
  {
    id: "motion",
    ...
    description: "Long exposures of tide, traffic, and crowds — the blur that proves time actually passed.",
    link: { href: "#", label: "View series" },
  },
  {
    id: "stillness",
    ...
    description: "Empty rooms at six in the morning. Nothing happening, on purpose.",
    link: { href: "#", label: "View series" },
  },
  {
    id: "depth",
  ...
  },
  {
    ...
    description: "What the light leaves out — negative space doing the actual storytelling.",
    link: { href: "#", label: "View series" },
  },
  {
    id: "scale",
  ...
  return (
    <div className="demo-page">
      ...
      <p className="demo-sub">
        <b>Hover a frame</b> — or tab to it and press <b>Enter</b> — to open the notes. Neighbouring
        frames make room by shrinking; the index itself never changes size.
      </p>

      <HoverBentoGrid tiles={tiles} columns={4} gap={14} radius={22} rowHeight={230} />

      <p className="demo-foot">01 — 08 · updated annually · scroll unaffected by hover state</p>
    </div>
  );
}