...
import { Calendar, Edit, Image as ImageIcon, Plus, Trash2, X } from "lucide-react";
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="flex justify-between items-center mb-10">
        <div>
          <h2 className="text-3xl font-display font-semibold text-navy-900 mb-2">Events</h2>
      ...
      {isFormOpen && (
        <div className="bg-white p-8 rounded-[2rem] border border-gray-200 mb-12 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Full Event Writeup</label>
              <textarea
                required
                value={writeup}
            ...
            <div>
              ...
              {existingImages.length > 0 && (
                <div className="flex gap-4 mb-4 overflow-x-auto pb-2">
                  {existingImages.map((img, idx) => (
                    <div key={idx} className="relative w-24 h-24 rounded-lg overflow-hidden flex-shrink-0 border border-gray-200">
                      <Image src={img} alt="Event image" fill className="object-cover" />
                      <button
                        type="button"
              ...
              <div className="relative border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:bg-gray-50 transition-colors">
                ...
                <p className="text-gray-600 font-medium">Click to upload images or drag and drop</p>
                {files && files.length > 0 && (
                  <p className="text-sm text-blue-600 mt-2">{files.length} file(s) selected</p>
                )}
              </div>
            </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-[2rem] border border-gray-200">
          ...
          <h3 className="text-xl font-medium text-navy-900 mb-2">No events found</h3>
          <p className="text-gray-500">Click "Add Event" to create your first newsletter event.</p>
        </div>
      ) : (
        <div className="grid gap-4">
          {events.map((event) => (
            <div key={event.id} className="bg-white p-6 rounded-2xl border border-gray-200 flex flex-col sm:flex-row gap-6 items-start sm:items-center justify-between group hover:shadow-md transition-all">
              <div className="flex items-center gap-6 flex-grow">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-navy-900 mb-1">{event.title}</h3>
